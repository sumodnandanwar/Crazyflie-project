...
def Yaw_callback(rposx,rposy):
    global ang, Yaw_cmd, lapp

    Yaw_cmd = Position()
    Yaw_cmd.header.frame_id = 'cf1/odom'  

    ang -= 5
    # if dist(cu_pose.pose.pose.position.x,rposx,cu_pose.pose.pose.position.y,rposy) < thr:
    #     Yaw_cmd.x = cu_pose.pose.pose.position.x
    #     Yaw_cmd.y = cu_pose.pose.pose.position.y
    # else: 
    Yaw_cmd.x = rposx
    Yaw_cmd.y = rposy

    if (cu_pose.pose.pose.position.z>= 0.375 and cu_pose.pose.pose.position.z <= 0.425):
        Yaw_cmd.z = cu_pose.pose.pose.position.z
    else:
        Yaw_cmd.z = 0.4
...
def goalpub(cmdx,cmdy):
    global odom_cmd, cmd, desired_angle
    ...
    yaww = round(yaww,3)
    desired_angle = -180-(math.degrees(yaww))
    ...
    if goal_odom:
        ...
        if state == 5:
            print state
            if desired_angle != cuyaw:
                cmd.yaw += 5
            # elif desired_angle == cuyaw:
            #     return
...
def state_machine():
    global state, initialize, x, lap, trig_msg, cuyaw, lapp
    ...
    curoll, cupitch, cuyaw = euler_from_quaternion((cu_pose.pose.pose.orientation.x,
                                              cu_pose.pose.pose.orientation.y,
                                              cu_pose.pose.pose.orientation.z,
                                              cu_pose.pose.pose.orientation.w), axes = "rzyx")
    cuyaw = round(cuyaw,3)
    ...
    elif state >= 2 and (zval >= 0.35 and zval <= 0.45) and lap >= 1: #Head to waypoint
        Dict_goal()
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if state == 1:
            ...
            print state            
            Yaw_callback(odom_cmd.x,odom_cmd.y)
            pub_cmd.publish(Yaw_cmd)
        elif state == 5:
            if desired_angle != cuyaw:
                cmd.yaw += 5
            pub_cmd.publish(cmd)
